...
else if (!isset($obj->edit_bank_pledger_id) && isset($obj->name)) {
...
else if (isset($obj->edit_bank_pledger_id)) {
    $edit_id = $obj->edit_bank_pledger_id;
    if (isset($obj->closing_date)) {
        // Closing update
        $closing_date = $obj->closing_date;
        $closing_amount = isset($obj->closing_amount) ? $obj->closing_amount : null;
        $closing_interest_amount = isset($obj->closing_interest_amount) ? $obj->closing_interest_amount : null;
        $extra_charges = isset($obj->extra_charges) ? $obj->extra_charges : null;
        $status = isset($obj->status) ? $obj->status : 'Closed';
        // For closing, also restore the bank limits
        // Fetch the record to get bank_pledger_details_id, bank_details, pawn_value
        $pledgerQuery = "SELECT `bank_pledger_details_id`, `bank_details`, `pawn_value` FROM `bank_pledger` WHERE `bank_pledge_id`='$edit_id' AND `delete_at` = 0";
        ...
        $updateBankPledger = "UPDATE `bank_pledger` SET `closing_date`='$closing_date', `closing_amount`='$closing_amount', `closing_interest_amount`='$closing_interest_amount', `extra_charges`='$extra_charges', `status`='$status' WHERE `bank_pledge_id`='$edit_id'";
        if ($conn->query($updateBankPledger)) {
            $output["head"]["code"] = 200;
            $output["head"]["msg"] = "Successfully bank_pledger Closed";
        } else {
            $output["head"]["code"] = 400;
            $output["head"]["msg"] = "Failed to close bank_pledger. Please try again.";
        }
    } else {
        // Full Edit update
        $bank_pledger_details_id = isset($obj->bank_pledger_details_id) ? $obj->bank_pledger_details_id : null;
...
// <<<<<<<<<<===================== This is to Delete the bank_pledgers =====================>>>>>>>>>>
    ...
    if (!empty($delete_bank_pledger_id)) {
        // Check if it's closed; if closed, don't restore limits
        $statusCheck = $conn->query("SELECT `status`, `bank_pledger_details_id`, `bank_details`, `pawn_value` FROM `bank_pledger` WHERE `bank_pledge_id`='$delete_bank_pledger_id' AND `delete_at` = 0");
        if ($statusCheck->num_rows > 0) {
            $statusRow = $statusCheck->fetch_assoc();
            if ($statusRow['status'] === 'Closed') {
                // If closed, just soft delete without restoring limits
                ...
                // Active, restore limits as before
                $bank_pledger_details_id = $statusRow['bank_pledger_details_id'];
                $bank_details_json = $statusRow['bank_details'];
                $pawn_value = $statusRow['pawn_value'];
                $new_bank_details_array = json_decode($bank_details_json, true);
                if (json_last_error() === JSON_ERROR_NONE && is_array($new_bank_details_array) && count($new_bank_details_array) > 0) {
                    $selected_bank_id = $new_bank_details_array[0]['bank_id'];
                    // Fetch current bank_pledger_details
